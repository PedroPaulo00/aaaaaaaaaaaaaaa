...
public class JwtUtil {
    ...
    public String gerarToken(Authentication authentication){
        ...
        Dono dono = (Dono) authentication.getPrincipal();
        try{
            Key secretKey = Keys.hmacShaKeyFor(jwtSecret.getBytes("UTF-8"));
            return Jwts.builder().setSubject(dono.getUsername()).setIssuedAt(new Date()).setExpiration(dataExpiracao).signWith(secretKey).compact();
